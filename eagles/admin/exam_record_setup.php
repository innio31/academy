<?php
// eagles/admin/exam_record_setup.php - Create / Edit Exam Record Setup (Class-first UX)
...
// Get class list for dropdown (all classes)
$all_classes = [];
foreach ($classes as $c) {
    $all_classes[] = $c['class_name'];
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_exam_record') {
        ...
        $default_promotion_class = trim($_POST['default_promotion_class'] ?? '');
        ...
        // Toggles - IMPORTANT: These come from POST, default to 0 if not set
        $show_class_pos       = isset($_POST['show_class_position']) && $_POST['show_class_position'] == '1' ? 1 : 0;
        $show_subject_pos     = isset($_POST['show_subject_position']) && $_POST['show_subject_position'] == '1' ? 1 : 0;
        $show_promoted        = isset($_POST['show_promoted_to']) && $_POST['show_promoted_to'] == '1' ? 1 : 0;
        $show_cum_avg         = isset($_POST['show_cumulative_avg']) && $_POST['show_cumulative_avg'] == '1' ? 1 : 0;
        $show_low_high_avg    = isset($_POST['show_lowest_highest_avg']) && $_POST['show_lowest_highest_avg'] == '1' ? 1 : 0;
        $show_low_high_class  = isset($_POST['show_lowest_highest_class']) && $_POST['show_lowest_highest_class'] == '1' ? 1 : 0;
        $sequential_pos       = isset($_POST['sequential_positions']) && $_POST['sequential_positions'] == '1' ? 1 : 0;
        $show_attendance      = isset($_POST['show_attendance']) && $_POST['show_attendance'] == '1' ? 1 : 0;
        $show_affective       = isset($_POST['show_affective_traits']) && $_POST['show_affective_traits'] == '1' ? 1 : 0;
        $show_psychomotor     = isset($_POST['show_psychomotor']) && $_POST['show_psychomotor'] == '1' ? 1 : 0;
                        ...
                        $stmt = $pdo->prepare("
                            INSERT INTO report_card_settings
                                (school_id, record_name, session, term, class, template,
                                 ...
                                 default_class_teacher_name, default_promotion_class,
                                 principal_comments_per_grade,
...
<body>
    ...
    <script>
        ...
        let allClassesList = <?php echo json_encode($all_classes); ?>;
        ...
        const sections = [{
                ...
                icon: 'fa-palette',
                description: 'Choose a visual style for report cards'
            },
            {
                id: 'record_details',
                title: 'Exam Record Details',
                icon: 'fa-info-circle',
                description: 'Basic information about this exam record'
            },
            {
                id: 'score_settings',
                title: 'Score Settings',
                icon: 'fa-sliders-h',
                description: 'Configure score components (must total 100)'
            },
            {
                id: 'grading_system',
                title: 'Grading System',
                icon: 'fa-award',
                description: 'Set grade boundaries and remarks'
            },
            {
                id: 'comments_setup',
                title: 'Comments Setup',
                icon: 'fa-comment-dots',
                description: 'Default class teacher and principal comments'
            },
            {
                id: 'term_dates',
                title: 'Term Dates',
                icon: 'fa-calendar-alt',
                description: 'Set term resumption and closing dates'
            },
            {
                id: 'display_options',
                title: 'Display Options',
                icon: 'fa-cog',
                description: 'Toggle report card sections'
            }
        ];
        ...
        function openCreateModal(className = '') {
            ...
            formData.default_promotion_class = '';
            ...
            formData.grading_scale = JSON.parse(JSON.stringify(gradingPresets.simple));
            ...
            fetch('exam_record_setup.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        formData.default_promotion_class = r.default_promotion_class || '';
                        ...
                        // Load display options from database correctly
                        ...
                        // Store all classes list from response
                        if (data.all_classes) {
                            allClassesList = data.all_classes;
                        }
        ...
        function markSectionCompleted(sectionId, value = true) {
            ...
            for (let item of sectionItems) {
                ...
                if (header) {
                    ...
                    const sectionName = sectionId.replace('_', ' ');
                    if (headerText.includes(sectionName) ||
                        (sectionId === 'display_options' && headerText.includes('display')) ||
                        (sectionId === 'record_details' && headerText.includes('exam record'))) {
                        if (value) {
        ...
        function renderRecordDetailsSection() {
            ...
            existingSessions.forEach(s => {
                sessionOptions += `<option value="${s.replace(/"/g, '&quot;')}">${s}</option>`;
            });

            let classOptions = '<option value="">— Select class —</option>';
            classesList.forEach(c => {
                classOptions += `<option value="${c.replace(/"/g, '&quot;')}" ${formData.class === c ? 'selected' : ''}>${c}</option>`;
            });
    ...
    `;
        ...
        function saveScoreSettings() {
            ...
            for (let i = 0; i < labels.length; i++) {
                ...
                if (label && max > 0) newScoreTypes.push({
                    label,
                    max
                });
            }
    ...
    `;
        ...
        function renderDisplayOptionsSection() {
            ...
            const options = [{
                    id: 'show_class_position',
                    label: 'Show class position'
                },
                {
                    id: 'show_subject_position',
                    label: 'Show subject position'
                },
                {
                    id: 'show_promoted_to',
                    label: 'Show "promoted to" next class'
                },
                {
                    id: 'show_cumulative_avg',
                    label: 'Show cumulative average'
                },
                {
                    id: 'show_lowest_highest_avg',
                    label: 'Show lowest & highest average'
                },
                {
                    id: 'show_lowest_highest_class',
                    label: 'Show lowest & highest in class'
                },
                {
                    id: 'sequential_positions',
                    label: 'Sequential positions (1st, 1st, 2nd)'
                },
                {
                    id: 'show_attendance',
                ...
                {
                    id: 'show_affective_traits',
                    label: 'Show affective traits'
                },
                {
                    id: 'show_psychomotor',
                    label: 'Show psychomotor skills'
                }
            ];

            let optionsHtml = '';
            options.forEach(opt => {
                const isChecked = formData[opt.id] === true;
                optionsHtml += `
            <div class="toggle-switch">
                <label>${opt.label}</label>
                <label class="switch">
                    <input type="checkbox" class="display-option" data-option="${opt.id}" ${isChecked ? 'checked' : ''}>
        ...
        `;
            ...
            // Add promotion class selection (shown only when show_promoted_to is enabled)
            const showPromotionClass = formData.show_promoted_to === true;
            const promotionClassHtml = `
        <div class="promotion-class-group" id="promotionClassGroup" style="${showPromotionClass ? '' : 'display: none;'}">
            <label><i class="fas fa-arrow-right"></i> Default Promotion Class:</label>
            <select id="defaultPromotionClass" style="width: 100%; margin-top: 8px;">
                <option value="">— Select promotion class —</option>
                ${allClassesList.map(c => `<option value="${c.replace(/"/g, '&quot;')}" ${formData.default_promotion_class === c ? 'selected' : ''}>${c}</option>`).join('')}
            </select>
            <small style="color:#666; display: block; margin-top: 5px;">This class will be suggested when promoting students. Can be overridden per student.</small>
        </div>
    `;
    ...
    `;
            ...
            // Add event listener to toggle promotion class visibility
            const promotedToggle = document.querySelector('.display-option[data-option="show_promoted_to"]');
            if (promotedToggle) {
                promotedToggle.addEventListener('change', function() {
                    const promotionGroup = document.getElementById('promotionClassGroup');
                    if (promotionGroup) {
                        promotionGroup.style.display = this.checked ? 'block' : 'none';
                    }
                });
            }
        }
        ...
        function saveDisplayOptions() {
            ...
            checkboxes.forEach(cb => {
                ...
                if (option) {
                    formData[option] = cb.checked;
                    console.log(`Saving ${option} = ${cb.checked}`);
                }
            });

            // Save promotion class
            const promotionClassSelect = document.getElementById('defaultPromotionClass');
            if (promotionClassSelect) {
                formData.default_promotion_class = promotionClassSelect.value;
            }
        ...
        function saveAllSections() {
            ...
            formDataObj.append('default_promotion_class', formData.default_promotion_class);
            ...
            // Send checkbox values as '1' or '0' strings
