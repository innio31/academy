...
{
    // Define the QR code file path - FIXED to include /eagles/
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/eagles/uploads/qrcodes/';
    ...
    if (file_exists($filepath)) {
        $qr_url = '/eagles/uploads/qrcodes/' . $filename;
...
{
    // FIXED: Added /eagles/ to the path
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/eagles/uploads/students/';
    ...
    if (optimizeImage($temp_path, $final_path, 100, 500, 500)) {
        ...
        return ['success' => true, 'path' => '/eagles/uploads/students/' . $filename];
    } else {
        unlink($temp_path);
