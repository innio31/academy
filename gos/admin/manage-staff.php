...
if (isset($_POST['rate_staff'])) {
    ...
    $stmt->execute([$staff_id, $school_id, $month, $punctuality, $task_completion, $student_feedback, $overall, $admin_id, $comments, 
                    $punctuality, $task_completion, $student_feedback, $overall, $comments]);
...
if (isset($_POST['assign_subjects'])) {
    ...
    foreach ($subjects as $subject_id) {
        $pdo->prepare("INSERT INTO staff_subjects (staff_id, subject_id, created_at) VALUES (?, ?, NOW())")->execute([$staff_id_string, $subject_id]);
    }
...
if (isset($_POST['assign_classes'])) {
    ...
    foreach ($classes as $class) {
        $pdo->prepare("INSERT INTO staff_classes (staff_id, class, created_at) VALUES (?, ?, NOW())")->execute([$staff_id_string, $class]);
    }
...
<head>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .logo-text h3 { font-size: 1rem; margin-bottom: 2px; }
    ...
    .admin-info h4 { font-size: 0.9rem; margin-bottom: 5px; }
    .admin-info p { font-size: 0.75rem; opacity: 0.8; }
    ...
    .nav-links a i { width: 20px; font-size: 1rem; }
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        <div class="logo">
            ...
            <div class="logo-text"><h3><?php echo $school_name; ?></h3><p>Admin Panel</p></div>
        </div>
        <div class="admin-info"><h4><?php echo htmlspecialchars($admin_name); ?></h4><p><?php echo ucfirst($admin_role); ?></p></div>
        <ul class="nav-links">
        ...
        <?php if ($action === 'list'): ?>
            <div class="filter-container">
                <form method="GET" class="filter-form">
                    ...
                    <div class="filter-group"><label class="form-label">Role</label><select name="role" class="form-select"><option value="">All</option><option value="staff">Staff</option><option value="admin">Admin</option></select></div>
                    <div class="filter-group"><label class="form-label">Status</label><select name="status" class="form-select"><option value="">All</option><option value="1">Active</option><option value="0">Inactive</option></select></div>
            ...
            <div class="table-container">
                <table class="data-table">
                    ...
                    <tbody>
                        ...
                        $params = [$school_id];
                        if (!empty($_GET['search'])) { $query .= " AND (full_name LIKE ? OR staff_id LIKE ? OR email LIKE ?)"; $s = "%{$_GET['search']}%"; array_push($params, $s, $s, $s); }
                        if ($_GET['role'] ?? '') { $query .= " AND role = ?"; $params[] = $_GET['role']; }
...
<td>
    <div class="action-buttons">
        ...
        <?php endif; ?>
            ...
            <div class="stats-grid">
                <?php
                $total = $pdo->prepare("SELECT COUNT(*) FROM staff WHERE school_id = ?"); $total->execute([$school_id]);
                $active = $pdo->prepare("SELECT COUNT(*) FROM staff WHERE school_id = ? AND is_active = 1"); $active->execute([$school_id]);
                $admin_count = $pdo->prepare("SELECT COUNT(*) FROM staff WHERE school_id = ? AND role = 'admin'"); $admin_count->execute([$school_id]);
                ?>
                <div class="stat-card"><div class="stat-value"><?php echo $total->fetchColumn(); ?></div><div>Total Staff</div></div>
                <div class="stat-card"><div class="stat-value"><?php echo $active->fetchColumn(); ?></div><div>Active Staff</div></div>
                <div class="stat-card"><div class="stat-value"><?php echo $admin_count->fetchColumn(); ?></div><div>Administrators</div></div>
            </div>
            ...
            <div class="form-container">
                ...
                <form method="POST">
                    ...
                    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px,1fr)); gap: 20px;">
                        ...
                        <div class="form-group"><label class="form-label">Role</label><select name="role" class="form-select"><option value="staff" <?php echo ($staff['role'] ?? '') === 'staff' ? 'selected' : ''; ?>>Staff</option><option value="admin" <?php echo ($staff['role'] ?? '') === 'admin' ? 'selected' : ''; ?>>Administrator</option></select></div>
        ...
        <?php elseif ($action === 'attendance'): ?>
            <div class="form-container">
                ...
                <div class="table-container">
                    <table class="data-table">
                        <thead><tr><th>Staff ID</th><th>Name</th><th>Clock In</th><th>Clock Out</th><th>Status</th><th>Action</th></tr></thead>
                        <tbody>
        ...
        <?php elseif ($action === 'class_attendance'): ?>
            <div class="form-container">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        ...
                        <form method="POST">
                            ...
                            <div class="form-group"><label>Session</label><select name="session_scan" class="form-select"><option value="morning">Morning</option><option value="afternoon">Afternoon</option></select></div>
                ...
                <div class="table-container">
                    <table class="data-table">
                        <thead><tr><th>Class</th><th>Staff</th><th>Time In</th><th>Time Out</th></tr></thead>
                        <tbody>
                            <?php
                            $today = date('Y-m-d');
                            ...
                            foreach ($stmt->fetchAll() as $ca): ?>
                                <tr><td><?php echo htmlspecialchars($ca['class_name']); ?></td><td><?php echo htmlspecialchars($ca['staff_name']); ?></td><td><?php echo $ca['time_in']; ?></td><td><?php echo $ca['time_out'] ?? '—'; ?></td></tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
                </div>
        ...
        <?php elseif ($action === 'performance'): ?>
            <div class="form-container">
                ...
                <div class="table-container">
                    <table class="data-table">
                        <thead><tr><th>Staff Name</th><th>Punctuality</th><th>Task Completion</th><th>Student Feedback</th><th>Overall Rating</th><th>Action</th></tr></thead>
                        <tbody>
                            <?php
    ...
    <div class="modal" id="rateModal">
        <div class="modal-content">
            <div class="modal-header"><h3>Rate Staff Performance</h3><button class="close-modal" onclick="closeRateModal()">&times;</button></div>
            <form method="POST">
