<?php
// staff/manage-topics.php - Staff Manage Topics (Full CRUD for assigned subjects)
session_start();
...
// Helper function to verify staff has access to subject
function verifyStaffSubjectAccess($pdo, $subject_id, $assigned_subject_ids, $school_id) {
    if (!in_array($subject_id, $assigned_subject_ids)) {
        throw new Exception("You don't have access to this subject");
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_manual_topic'])) {
    try {
        ...
        $subject = verifyStaffSubjectAccess($pdo, $subject_id_post, $assigned_subject_ids, $school_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_topic'])) {
    ...
    $term = $_POST['edit_term'] ?? 'First';

    try {
        // Get the subject_id for this topic and verify access
        $stmt = $pdo->prepare("SELECT subject_id, topic_name FROM topics WHERE id = ? AND school_id = ? AND is_central = 0");
        ...
        verifyStaffSubjectAccess($pdo, $topic_info['subject_id'], $assigned_subject_ids, $school_id);

        if (empty($topic_name)) {
            throw new Exception("Topic name cannot be empty");
        }

        // Check if new topic name already exists for this subject (excluding current topic)
        $stmt = $pdo->prepare("SELECT id FROM topics WHERE topic_name = ? AND subject_id = ? AND school_id = ? AND id != ?");
        $stmt->execute([$topic_name, $topic_info['subject_id'], $school_id, $topic_id_edit]);
        if ($stmt->fetch()) {
            throw new Exception("Topic '{$topic_name}' already exists for this subject");
        }

        $stmt = $pdo->prepare("UPDATE topics SET topic_name = ?, description = ?, term = ? WHERE id = ? AND school_id = ?");
        $stmt->execute([$topic_name, $description, $term, $topic_id_edit, $school_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_multiple_topics'])) {
    try {
        ...
        if ($added_count > 0) {
            $message = "$added_count topic(s) added successfully to {$subject['subject_name']}";
            if ($skipped_count > 0) {
...
// Get all subjects assigned to staff for filter
...
// Get available central topics for selected subject
$available_central_topics = [];
if ($subject_id && in_array($subject_id, $assigned_subject_ids)) {
...
// Build topics query - only for assigned subjects
$query = "
...
$params = [$school_id];
$params = array_merge($params, $assigned_subject_ids);
...
if ($view_topic_id) {
    try {
        ...
        if ($view_topic) {
            ...
            if (!in_array($view_topic['subject_id'], $assigned_subject_ids)) {
                header("Location: manage-topics.php?message=Access+denied&type=error");
                exit();
            }
...
<body>
        ...
        <?php if ($view_topic): ?>
            ...
            <div class="subject-info-card">
                ...
                <div style="margin-top: 20px;">
                    <a href="add_questions.php?topic_id=<?php echo $view_topic_id; ?>" class="btn btn-success">
                        ...
                        <?php foreach ($objective_questions as $q): ?>
                            <div class="topic-card" style="cursor: default;">
                                ...
                                <div class="modal-action-buttons" style="margin-top: 12px; border-top: none; padding-top: 0;">
                                    <a href="manage-questions.php?topic_id=<?php echo $view_topic_id; ?>" class="btn btn-info modal-action-btn"><i class="fas fa-eye"></i> View Details</a>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
        ...
        <div class="modal-content">
            ...
            <form method="POST">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label>Term</label>
                        <select id="edit_term" name="edit_term" class="form-control">
                            <option value="First">First Term</option>
                            <option value="Second">Second Term</option>
                            <option value="Third">Third Term</option>
                        </select>
    ...
    <script>
                    ...
                    if (data.success) {
                        ...
                        modalBody.innerHTML = `
                            ...
                            <div class="modal-action-buttons">
                                ...
                                <button class="btn btn-warning modal-action-btn" onclick="closeModal('topicModal'); editTopic(${topic.id}, '${escapeHtml(topic.topic_name)}', '${escapeHtml(topic.term || 'First')}', '${escapeHtml(topic.description || '')}')"><i class="fas fa-edit"></i> Edit</button>
                                <a href="add_questions.php?topic_id=${topic.id}" class="btn btn-success modal-action-btn"><i class="fas fa-plus-circle"></i> Add Questions</a>
        ...
        function editTopic(id, name, term, description) {
