<?php
// gsa/admin/exam_broadsheet.php - Generate Class and Subject Broadsheets
...
if ($format === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="broadsheet_' . $class . '_' . $term . '_' . $session . '.csv"');
...
<button class="btn-pdf" onclick="downloadPDF()">
    ...
    <script>
        ...
        async function downloadPDF() {
            ...
            if (!container) { alert('No content to export'); return; }
            
            const btn = event.target.closest('button');
            if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Generating...'; }
            
            const hideEls = document.querySelectorAll('.no-print, .sidebar, .mobile-toggle, .overlay, .top-header, .control-bar, .footer, button, .btn');
            const originalDisplays = [];
            hideEls.forEach((el, i) => { originalDisplays[i] = el.style.display; el.style.display = 'none'; });
            
            try {
                const canvas = await html2canvas(container, { scale: 2.5, useCORS: true, backgroundColor: '#ffffff', logging: false });
                ...
                pdf.save(`broadsheet_${Date.now()}.pdf`);
