...
{
    // Define the QR code file path - FIXED to include /gsa/
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/gsa/uploads/qrcodes/';
    ...
    if (file_exists($filepath)) {
        $qr_url = '/gsa/uploads/qrcodes/' . $filename;
...
{
    // FIXED: Added /gsa/ to the path
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/gsa/uploads/students/';
    ...
    if (optimizeImage($temp_path, $final_path, 100, 500, 500)) {
        ...
        return ['success' => true, 'path' => '/gsa/uploads/students/' . $filename];
    } else {
        unlink($temp_path);
