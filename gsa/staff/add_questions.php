<?php
// staff/add_questions.php - Staff Add Questions (with CSV, WAEC, AI features)
session_start();
...
// Load selected topic and verify staff has access
$selected_topic = null;
if ($topic_id) {
    try {
        $stmt = $pdo->prepare("
...
<body>
    ...
    <script>
        ...
        function loadWAECSubjects() {
            ...
            fetch(`add_questions.php?ajax=waec_subjects`)
                .then(res => res.json())
                .then(data => {
                    if (data.success) {
        ...
        function loadWAECTopics() {
            ...
            fetch(`add_questions.php?ajax=waec_topics&subject_id=${sid}`)
                .then(res => res.json())
                .then(data => {
        ...
        function loadWAECQuestions() {
            ...
            let url = `add_questions.php?ajax=waec_questions&topic_id=${tid}`;
        ...
        function generateAIPreview() {
            ...
            fetch(`add_questions.php?ajax=ai_generate&subject_name=${encodeURIComponent(subject)}&topic_name=${encodeURIComponent(topic)}&class_level=${encodeURIComponent(classLevel)}&count=${count}`)
                .then(res => res.json())
                .then(data => {
