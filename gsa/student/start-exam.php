<?php
// gsa/student/start-exam.php  — Full-screen locked exam interface
session_start();
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
        ...
        .school-badge { display:flex; align-items:center; gap:10px; }
        .school-badge-icon {
        ...
        .calc-toggle-btn i { font-size:.85rem; }
        ...
        .question-text { font-size:1.02rem; line-height:1.7; color:var(--text); font-weight:500; }
        ...
        .calc-panel {
            position:fixed;
            bottom:24px; right:280px;          /* sits just left of the sidebar */
            width:320px;
            background:var(--surface);
            border:1px solid var(--border);
            border-radius:var(--radius);
        ...
        .calc-close-btn:hover { color:var(--danger); }
        ...
        .calc-expr {
            font-size:.72rem; color:var(--muted); font-family:'Courier New', monospace;
        ...
        .calc-screen.error { color:var(--danger); font-size:1.1rem; }
        ...
        .calc-btn:active { background:#1a2840; }

        /* Button colour variants */
        .calc-btn.op     { color:var(--secondary); }
        .calc-btn.fn     { color:#a78bfa; font-size:.7rem; }
        .calc-btn.eq     {
            background:linear-gradient(135deg, var(--secondary), var(--primary));
            color:#fff; grid-column:span 1;
        }
        .calc-btn.eq:hover { opacity:.88; }
        .calc-btn.clr    { color:var(--danger); }
        .calc-btn.mem    { color:var(--success); font-size:.7rem; }
        ...
        .calc-mode-btn.active { background:var(--primary); color:#fff; }
        ...
        @media (max-width:900px) {
            ...
            .calc-panel { right:12px; bottom:12px; width:300px; }
        }
    </style>
</head>
...
document.addEventListener('keydown', function (e) {
    ...
    else if (k === 'c')  { e.preventDefault(); toggleCalc(); }
...
function calcIsOpen() { return calcPanel.classList.contains('open'); }
...
function calcInput(token) {
    ...
    if (token === 'Math.PI') { calcBuffer += 'π'; }
    else if (token === 'Math.E') { calcBuffer += 'e'; }
    else if (token === '^') { calcBuffer += '^'; }
    else { calcBuffer += token; }
...
function calcFn(fn) {
    ...
    try {
        switch (fn) {
            ...
            case 'asin': result = angleMode === 'deg' ? toDeg(Math.asin(val)) : Math.asin(val); break;
            case 'acos': result = angleMode === 'deg' ? toDeg(Math.acos(val)) : Math.acos(val); break;
            case 'atan': result = angleMode === 'deg' ? toDeg(Math.atan(val)) : Math.atan(val); break;
...
function calcMem(op) {
    ...
    switch (op) {
        ...
        case 'MR': calcBuffer = String(calcMemory); calcScreen.textContent = calcBuffer; break;
