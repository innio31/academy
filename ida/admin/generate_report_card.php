...
if (!$record_id) { header("Location: exam_record_setup.php"); exit(); }
...
if (!$record || ($record['status'] ?? '') === 'archived') {
    header("Location: exam_record_setup.php"); exit();
}
...
} catch (Exception $e) { error_log("generate students: ".$e->getMessage()); }
...
} catch (Exception $e) { error_log("generate subjects: ".$e->getMessage()); }
...
} catch (Exception $e) { error_log("generate scores: ".$e->getMessage()); }
...
} catch (Exception $e) { error_log("generate comments/traits: ".$e->getMessage()); }
...
foreach ($students as $i => $s) {
    if ((int)$s['id'] === $preview_sid) { $preview_student = $s; $preview_idx = $i; break; }
}
...
function renderCard(
    array $stu, array $subjects, array $all_scores, array $all_comments,
    array $all_affective, array $all_psychomotor, array $all_positions,
    array $subject_stats, array $grading_scale, array $score_types,
    array $record, array $school, array $theme, string $school_display,
    float $highest_avg, float $lowest_avg, float $class_avg_mean, int $total_students
): string {
    $sid      = (int)$stu['id'];
    ...
    // Traits
    $af_fields = ['punctuality'=>'Punctuality','attendance'=>'Attendance','politeness'=>'Politeness',
                  'honesty'=>'Honesty','neatness'=>'Neatness','reliability'=>'Reliability',
                  'relationship'=>'Relationship','self_control'=>'Self Control'];
    $pm_fields = ['handwriting'=>'Handwriting','verbal_fluency'=>'Verbal Fluency','sports'=>'Sports',
                  'handling_tools'=>'Handling Tools','drawing_painting'=>'Drawing/Painting','musical_skills'=>'Musical Skills'];
    ...
    <div style="display:flex;padding:10px 20px;gap:12px;flex-wrap:wrap;border-bottom:1px solid #eee;">
        ...
        <div style="min-width:130px;">
            ...
            <table style="border-collapse:collapse;font-size:10px;">
                <tr><td style="padding:3px 4px;color:#555;">School opened:</td><td style="padding:3px 4px;font-weight:700;"><?php echo $days_opened; ?> days</td></tr>
                <tr><td style="padding:3px 4px;color:#555;">Days present:</td><td style="padding:3px 4px;font-weight:700;color:#27ae60;"><?php echo $days_present; ?></td></tr>
                <tr><td style="padding:3px 4px;color:#555;">Days absent:</td><td style="padding:3px 4px;font-weight:700;color:#e74c3c;"><?php echo $days_absent; ?></td></tr>
            </table>
        </div>
        <?php endif; ?>
    </div>
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .print-toolbar { background:#1a3c5e; color:white; padding:12px 20px; display:flex; gap:12px; align-items:center; }
        .print-toolbar button { padding:8px 16px; border:none; border-radius:6px; cursor:pointer; font-size:13px; font-weight:500; }
        .print-toolbar .btn-print { background:white; color:#1a3c5e; }
        .print-toolbar .btn-close { background:rgba(255,255,255,.2); color:white; }
...
<div class="cards-wrap">
    <?php foreach ($students_to_print as $stu):
        echo renderCard($stu, $subjects, $all_scores, $all_comments, $all_affective,
            $all_psychomotor, $all_positions, $subject_stats, $grading_scale,
            $score_types, $record, $school, $theme, $school_display,
            (float)$highest_avg, (float)$lowest_avg, (float)$class_avg_mean, $total_students);
    endforeach; ?>
</div>
<script>
...
<head>
    ...
    <style>
        :root{--primary:<?php echo $primary_color;?>;--secondary:<?php echo $secondary_color;?>;--success:#27ae60;--warning:#f39c12;--danger:#e74c3c;--light:#ecf0f1;--dark:#2c3e50;--sidebar-w:260px;--shadow:0 2px 8px rgba(0,0,0,.08);--radius:10px;--tr:all .25s ease;}
        *{margin:0;padding:0;box-sizing:border-box;}
        body{font-family:'Poppins',sans-serif;background:#f5f6fa;color:#333;min-height:100vh;overflow-x:hidden;}
        .sidebar{position:fixed;top:0;left:0;width:var(--sidebar-w);height:100vh;background:linear-gradient(180deg,var(--primary),var(--dark));color:white;padding:20px 0;transition:transform .3s;z-index:1000;overflow-y:auto;transform:translateX(-100%);}
        .sidebar.open{transform:translateX(0);}
        .sidebar-header{padding:0 20px 20px;border-bottom:1px solid rgba(255,255,255,.1);}
        .logo{display:flex;align-items:center;gap:12px;}
        .logo-icon{width:44px;height:44px;background:var(--secondary);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:20px;}
        .logo-text h3{font-size:1rem;font-weight:600;}
        .logo-text p{font-size:.7rem;opacity:.8;}
        .admin-info{text-align:center;padding:15px;background:rgba(255,255,255,.1);border-radius:10px;margin:15px;}
        .nav-links{list-style:none;padding:0 15px;}
        .nav-links li{margin-bottom:5px;}
        .nav-links a{display:flex;align-items:center;gap:12px;padding:12px 15px;color:rgba(255,255,255,.9);text-decoration:none;border-radius:8px;transition:var(--tr);}
        .nav-links a:hover{background:rgba(255,255,255,.15);}
        .nav-links a.active{background:rgba(255,255,255,.2);border-left:3px solid var(--secondary);}
        .nav-links i{width:20px;text-align:center;}
        .mob-btn{position:fixed;top:15px;left:15px;z-index:1001;width:44px;height:44px;background:var(--primary);color:white;border:none;border-radius:10px;font-size:20px;cursor:pointer;}
        .overlay{position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:999;opacity:0;visibility:hidden;transition:var(--tr);}
        .overlay.show{opacity:1;visibility:visible;}
        .main{min-height:100vh;padding:20px;transition:var(--tr);}
        .top-header{background:white;padding:18px 20px;border-radius:var(--radius);margin-bottom:20px;display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px;box-shadow:var(--shadow);}
        .top-header h1{color:var(--primary);font-size:1.35rem;margin-bottom:3px;}
        .top-header p{color:#777;font-size:.82rem;}
        .back-btn{background:white;border:1px solid var(--light);color:var(--primary);padding:9px 16px;border-radius:8px;display:inline-flex;align-items:center;gap:8px;font-family:'Poppins',sans-serif;font-size:.83rem;text-decoration:none;}
        .step-bar{display:flex;background:white;border-radius:var(--radius);padding:14px 20px;margin-bottom:20px;box-shadow:var(--shadow);overflow-x:auto;}
        .step-item{flex:1;display:flex;flex-direction:column;align-items:center;min-width:80px;position:relative;}
        .step-item:not(:last-child)::after{content:'';position:absolute;top:14px;left:50%;width:100%;height:2px;background:var(--light);z-index:0;}
        .step-circle{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600;z-index:1;position:relative;border:2px solid transparent;}
        ...
        .step-lbl{font-size:10px;color:#999;margin-top:5px;text-align:center;}
        .alert{padding:13px 16px;border-radius:8px;margin-bottom:18px;font-size:.86rem;display:flex;align-items:flex-start;gap:10px;}
        ...
        .stat-box{background:white;border-radius:var(--radius);padding:14px 16px;box-shadow:var(--shadow);border-top:3px solid var(--primary);}
        .stat-box.green{border-top-color:var(--success);}.stat-box.amber{border-top-color:var(--warning);}.stat-box.red{border-top-color:var(--danger);}
        .stat-val{font-size:1.5rem;font-weight:700;color:var(--primary);}
        .stat-box.green .stat-val{color:var(--success);}.stat-box.amber .stat-val{color:var(--warning);}.stat-box.red .stat-val{color:var(--danger);}
        .stat-lbl{font-size:.74rem;color:#777;margin-top:2px;}
        .content-grid{display:grid;grid-template-columns:230px 1fr;gap:20px;align-items:start;}
        .student-panel{background:white;border-radius:var(--radius);box-shadow:var(--shadow);overflow:hidden;position:sticky;top:20px;}
        .panel-hdr{background:var(--primary);color:white;padding:14px 16px;font-size:.88rem;font-weight:600;}
        .s-list{list-style:none;padding:6px 0;max-height:68vh;overflow-y:auto;}
        .s-list li a{display:flex;align-items:center;gap:8px;padding:9px 16px;color:#333;text-decoration:none;font-size:.82rem;transition:background .15s;}
        .s-list li a:hover{background:#f5f6fa;}
        .s-list li a.active{background:#eef3ff;color:var(--primary);border-left:3px solid var(--primary);font-weight:600;}
        .s-av{width:26px;height:26px;border-radius:50%;background:var(--primary);color:white;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;}
        .pos-badge{font-size:.7rem;padding:1px 6px;border-radius:10px;background:var(--light);color:#555;margin-left:auto;flex-shrink:0;}
        .pos-badge.set{background:#d4edda;color:#155724;}
        .panel-foot{font-size:.72rem;color:#888;padding:8px 16px;border-top:1px solid var(--light);}
        .action-card{background:white;border-radius:var(--radius);box-shadow:var(--shadow);padding:18px 20px;margin-bottom:20px;}
        .action-card h2{font-size:.95rem;font-weight:600;color:var(--primary);margin-bottom:14px;display:flex;align-items:center;gap:8px;}
        .btn{padding:9px 18px;border-radius:8px;font-family:'Poppins',sans-serif;font-size:.85rem;font-weight:500;cursor:pointer;border:none;display:inline-flex;align-items:center;gap:7px;text-decoration:none;transition:var(--tr);}
        .btn-primary{background:var(--primary);color:white;}.btn-primary:hover{opacity:.9;}
        .btn-secondary{background:white;color:var(--primary);border:1.5px solid var(--primary);}.btn-secondary:hover{background:var(--primary);color:white;}
        .btn-success{background:var(--success);color:white;}
        .btn-warning{background:var(--warning);color:white;}
        .btn-sm{padding:6px 12px;font-size:.78rem;}
        .btn-row{display:flex;gap:10px;flex-wrap:wrap;align-items:center;}
        .preview-wrap{background:white;border-radius:var(--radius);box-shadow:var(--shadow);padding:20px;overflow-x:auto;}
        .preview-nav{display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:10px;margin-bottom:16px;}
        .preview-nav h2{font-size:.95rem;font-weight:600;color:var(--primary);display:flex;align-items:center;gap:8px;}
        .checklist{display:flex;flex-direction:column;gap:8px;margin-bottom:16px;}
        .check-item{display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:8px;background:#f9f9f9;font-size:.83rem;}
        ...
        .ci-ok{background:#d4edda;color:#155724;}.ci-warn{background:#fff3cd;color:#856404;}.ci-bad{background:#f8d7da;color:#721c24;}
        .tpl-badge{display:inline-flex;align-items:center;gap:6px;padding:5px 12px;border-radius:20px;font-size:.78rem;font-weight:500;background:var(--light);}
        @media(min-width:768px){.mob-btn,.overlay{display:none;}.sidebar{transform:translateX(0);}.main{margin-left:var(--sidebar-w);}}
        @media(max-width:960px){.content-grid{grid-template-columns:1fr;}.student-panel{position:static;}.s-list{max-height:none;display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));}}
        @media(max-width:767px){.main{padding-top:70px;}.stats-row{grid-template-columns:1fr 1fr;}}
    </style>
</head>
<body>
...
<div class="sidebar" id="sidebar">
    <div class="sidebar-header">
        <div class="logo">
            ...
            <div class="logo-text"><h3><?php echo htmlspecialchars($school_name_const); ?></h3><p>Admin Panel</p></div>
        </div>
    </div>
...
<div class="main" id="mainContent">
    ...
    <div class="step-bar">
        ...
        <div class="step-item"><div class="step-circle s-done"><i class="fas fa-check" style="font-size:10px"></i></div><div class="step-lbl">Traits &amp; comments</div></div>
        <div class="step-item"><div class="step-circle s-cur">4</div><div class="step-lbl">Generate cards</div></div>
        <div class="step-item"><div class="step-circle s-todo">5</div><div class="step-lbl">Publish</div></div>
    </div>

    <!-- Stats -->
    <div class="stats-row">
        <div class="stat-box"><div class="stat-val"><?php echo $total_students; ?></div><div class="stat-lbl">Students</div></div>
        <div class="stat-box <?php echo $students_with_scores >= $total_students ? 'green' : 'amber'; ?>">
            <div class="stat-val"><?php echo $students_with_scores; ?></div><div class="stat-lbl">With scores</div></div>
        <div class="stat-box <?php echo $students_with_comments >= $total_students ? 'green' : 'amber'; ?>">
            <div class="stat-val"><?php echo $students_with_comments; ?></div><div class="stat-lbl">With comments</div></div>
        <div class="stat-box <?php echo $positions_exist ? 'green' : 'red'; ?>">
            <div class="stat-val"><?php echo $students_with_positions; ?></div><div class="stat-lbl">Positions set</div></div>
        <div class="stat-box">
            <div class="stat-val"><?php echo $class_avg_mean > 0 ? number_format($class_avg_mean,1).'%' : '—'; ?></div><div class="stat-lbl">Class average</div></div>
        <div class="stat-box green">
            <div class="stat-val"><?php echo $highest_avg > 0 ? number_format($highest_avg,1).'%' : '—'; ?></div><div class="stat-lbl">Highest avg.</div></div>
    </div>
        ...
        <div>
            ...
            <div class="preview-wrap">
                ...
                <div style="transform-origin:top left;overflow-x:auto;">
                    <?php echo renderCard(
                        $preview_student, $subjects, $all_scores, $all_comments,
                        $all_affective, $all_psychomotor, $all_positions,
                        $subject_stats, $grading_scale, $score_types,
                        $record, $school, $theme, $school_display,
                        (float)$highest_avg, (float)$lowest_avg, (float)$class_avg_mean,
                        $total_students
                    ); ?>
                </div>
            </div>
...
btn.addEventListener('click', () => { sb.classList.toggle('open'); ov.classList.toggle('show'); document.body.style.overflow = sb.classList.contains('open') ? 'hidden' : ''; });
ov.addEventListener('click',  () => { sb.classList.remove('open'); ov.classList.remove('show'); document.body.style.overflow = ''; });
</script>
