...
// Fetch full school record for address, phone, email, motto
$stmt = $pdo->prepare("SELECT * FROM schools WHERE id = ? LIMIT 1");
$stmt->execute([$school_id]);
$school = $stmt->fetch();

$school_name    = $school['school_name'] ?? SCHOOL_NAME;
$school_address = $school['contact_email'] ?? '';   // re-used below properly
...
$template   = $_GET['template']   ?? ''; // empty = show picker
