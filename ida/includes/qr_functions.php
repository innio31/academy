...
{
    // Define the QR code file path - FIXED to include /ida/
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/ida/uploads/qrcodes/';
    ...
    if (file_exists($filepath)) {
        $qr_url = '/ida/uploads/qrcodes/' . $filename;
...
{
    // FIXED: Added /ida/ to the path
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/ida/uploads/students/';
    ...
    if (optimizeImage($temp_path, $final_path, 100, 500, 500)) {
        ...
        return ['success' => true, 'path' => '/ida/uploads/students/' . $filename];
    } else {
        unlink($temp_path);
