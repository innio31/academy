...
function saveStudentQRCode($pdo, $student_id, $qr_data) {
    // Define the QR code file path - FIXED to include /gos/
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/gos/uploads/qrcodes/';
    ...
    if (file_exists($filepath)) {
        $qr_url = '/gos/uploads/qrcodes/' . $filename;
...
function uploadStudentImage($file, $student_id) {
    // FIXED: Added /gos/ to the path
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/gos/uploads/students/';
    ...
    if (optimizeImage($temp_path, $final_path, 100, 500, 500)) {
        ...
        return ['success' => true, 'path' => '/gos/uploads/students/' . $filename];
    } else {
        unlink($temp_path);
