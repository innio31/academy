...
<body>
    ...
    foreach ($logo_paths_to_try as $logo_path) {
        if (!empty($logo_path) && file_exists($_SERVER['DOCUMENT_ROOT'] . $logo_path)) {
            ...
            $logo_displayed = true;
            break;
        } elseif (!empty($logo_path) && file_exists($_SERVER['DOCUMENT_ROOT'] . '/msv/' . $logo_path)) {
            echo '<img class="rc-logo" src="/msv/' . htmlspecialchars($logo_path) . '" alt="School Logo" onerror="this.style.display=\'none\'">';
            $logo_displayed = true;
            break;
