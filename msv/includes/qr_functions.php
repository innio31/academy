...
{
    // Define the QR code file path - FIXED to include /msv/
...
{
    // FIXED: Added /msv/ to the path
    $upload_dir = $_SERVER['DOCUMENT_ROOT'] . '/msv/uploads/students/';
    ...
    if (optimizeImage($temp_path, $final_path, 100, 500, 500)) {
        ...
        return ['success' => true, 'path' => '/msv/uploads/students/' . $filename];
    } else {
        unlink($temp_path);
