...
<body>
    ...
    <nav class="sidebar" id="sidebar">
        ...
        <div class="admin-info">
            <div style="width:52px;height:52px;background:var(--secondary);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:1.4rem;margin:0 auto 6px;">
        ...
        <ul class="nav-links">
            ...
            <li><a href="manage-students.php"><i class="fas fa-user-graduate"></i> Students</a></li>
    ...
    <button class="mobile-toggle" id="menuBtn" aria-label="Menu"><i class="fas fa-bars"></i></button>
    ...
    <main class="main">
        ...
        <div class="top-header no-print">
            ...
            <a href="exam_traits_comments.php?record_id=<?php echo $record_id; ?>" class="back-btn"><i class="fas fa-arrow-left"></i> Back to Step 3</a>
        </div>

        <!-- Step bar -->
        <div class="step-bar no-print">
            <div class="step-item">
                <div class="step-circle s-done"><i class="fas fa-check"></i></div><span class="step-lbl">Setup</span>
            </div>
            <div class="step-item">
                <div class="step-circle s-done"><i class="fas fa-check"></i></div><span class="step-lbl">Scores</span>
            </div>
            <div class="step-item">
                <div class="step-circle s-done"><i class="fas fa-check"></i></div><span class="step-lbl">Traits</span>
            </div>
            <div class="step-item">
                <div class="step-circle s-cur">4</div><span class="step-lbl">Cards</span>
            </div>
        </div>

        <!-- Flash messages -->
        <?php if ($success_msg): ?>
            <div class="alert alert-success no-print"><i class="fas fa-check-circle"></i>
                <div><?php echo htmlspecialchars($success_msg); ?></div>
            </div>
        <?php endif; ?>
        <?php if ($error_msg): ?>
            <div class="alert alert-danger no-print"><i class="fas fa-exclamation-circle"></i>
        ...
        <?php if (empty($students)): ?>
            <div class="alert alert-warning"><i class="fas fa-exclamation-triangle"></i>
                <div>No active students found for <strong><?php echo htmlspecialchars($class); ?></strong>. Please ensure students are enrolled and active.</div>
            </div>
        <?php else: ?>
            ...
            <div class="stats-row no-print">
                ...
                <div class="stat-box">
                    ...
                    <div class="stat-lbl">Lowest avg</div>
                </div>
            </div>
            ...
            <div class="ready-bar no-print">
                ...
                <div class="ready-checks">
                    ...
                    <div class="ready-check <?php echo !empty($affective) ? 'ok' : 'warn'; ?>">
                        ...
                        Affective: <?php echo count($affective); ?> students
                    </div>
                    <div class="ready-check <?php echo !empty($psychomotor) ? 'ok' : 'warn'; ?>">
                        ...
                        Psychomotor: <?php echo count($psychomotor); ?>
                    </div>
                    <div class="ready-check <?php echo ($record['status'] ?? '') === 'published' ? 'ok' : 'warn'; ?>">
                        ...
                        Status: <span class="status-pill <?php echo ['draft' => 'pill-draft', 'active' => 'pill-active', 'published' => 'pill-published'][$record['status'] ?? 'draft'] ?? 'pill-draft'; ?>"><?php echo ucfirst($record['status'] ?? 'draft'); ?></span>
                    </div>
                </div>
            </div>
                ...
                <div style="display:flex;gap:10px;flex-wrap:wrap;">
                    <?php if (($record['status'] ?? '') !== 'published'): ?>
                        <form method="POST" style="display:inline" onsubmit="return confirm('Publish all report cards? Students and parents will be able to view them.')">
                            ...
                            <button type="submit" class="btn btn-success btn-sm" <?php echo !$all_ready ? 'title="Some students are missing scores or comments"' : ''; ?>><i class="fas fa-globe"></i> Publish cards</button>
                        </form>
                    <?php else: ?>
                    ...
                    <?php endif; ?>
                    <button class="btn btn-secondary btn-sm" onclick="window.print()"><i class="fas fa-print"></i> Print this card</button>
                    <button class="btn btn-primary btn-sm" id="downloadPdfBtn" onclick="downloadReportCardPDF()"><i class="fas fa-file-pdf"></i> Download PDF</button>
                    <a href="exam_record_setup.php" class="btn btn-secondary btn-sm"><i class="fas fa-list"></i> All records</a>
                </div>
            </div>
            ...
            <div class="layout-grid">
                ...
                <div class="student-panel no-print">
                    <div class="panel-head"><i class="fas fa-users"></i> Students (<?php echo $total_students; ?>)</div>
                    <div class="panel-search"><input type="text" id="studentSearch" placeholder="🔍 Search student..." onkeyup="filterStudents()"></div>
                    <ul class="student-list" id="studentList">
                        ...
                        ?>
                            <li data-name="<?php echo htmlspecialchars(strtolower($s['full_name'])); ?>">
                                <a href="?record_id=<?php echo $record_id; ?>&student_id=<?php echo $sid; ?>" class="<?php echo $is_active ? 'active' : ''; ?>">
                                    <div class="s-avatar">
                                    ...
                                    <div class="s-info">
                                        ...
                                        <span><?php echo htmlspecialchars($s['admission_number']); ?> &bull; Pos: <?php echo $pos_str; ?></span>
                                    </div>
                                    <span class="s-badge <?php echo ($has_scores && $has_comm) ? '' : 'missing'; ?>"><?php echo ($has_scores && $has_comm) ? 'Ready' : 'Incomplete'; ?></span>
                                </a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                ...
                <div class="card-wrapper">
                    <div class="card-toolbar no-print">
                        <div class="card-toolbar-left"><i class="fas fa-eye" style="color:var(--primary)"></i> <?php if ($preview_student): ?>Report Card — <strong><?php echo htmlspecialchars($preview_student['full_name']); ?></strong><?php else: ?>Select a student<?php endif; ?></div>
                        <div class="card-toolbar-right">
                            <?php if ($preview_student): ?>
                                ...
                                <button class="btn btn-primary btn-sm" onclick="downloadReportCardPDF()"><i class="fas fa-file-pdf"></i> Download PDF</button>
                            <?php endif; ?>
                        </div>
                    </div>
                    ...
                    ?>
                        <div class="rc-wrap">
                            <div class="rc-card" id="reportCard">
                                <!-- Header -->
                                <div class="rc-header">
                                    <img class="rc-logo" src="<?php echo htmlspecialchars($school_logo); ?>" alt="<?php echo htmlspecialchars($school_name); ?>" onerror="this.style.display='none'">
                                    <div class="rc-school-info">
                                        <h2><?php echo htmlspecialchars($school_name); ?></h2>
                                        <?php if ($school_email || $school_phone): ?>
                                            <p><?php echo htmlspecialchars($school_email); ?><?php echo ($school_email && $school_phone) ? ' &bull; ' : ''; ?><?php echo htmlspecialchars($school_phone); ?></p>
                                        <?php endif; ?>
                                        <?php if ($school_motto): ?>
                                            <p style="font-style:italic;color:var(--secondary)">"<?php echo htmlspecialchars($school_motto); ?>"</p>
                                        <?php endif; ?>
                                        <div class="rc-title">Student Report Card — <?php echo htmlspecialchars($term); ?> Term</div>
                                    </div>
                                    <?php if (!empty($preview_student['profile_picture'])): ?>
                                        <img class="rc-photo" src="<?php echo htmlspecialchars($preview_student['profile_picture']); ?>" alt="Photo" onerror="this.style.display='none'">
                                    <?php else: ?>
                                ...
                                <!-- Bio -->
                                <div class="rc-bio">
                                    <div class="rc-bio-item"><span class="rc-bio-lbl">Student Name:</span><span class="rc-bio-val"><?php echo htmlspecialchars($preview_student['full_name']); ?></span></div>
                                    <div class="rc-bio-item"><span class="rc-bio-lbl">Admission No:</span><span class="rc-bio-val"><?php echo htmlspecialchars($preview_student['admission_number']); ?></span></div>
                                    ...
                                    <div class="rc-bio-item"><span class="rc-bio-lbl">Gender:</span><span class="rc-bio-val"><?php echo htmlspecialchars(ucfirst($preview_student['gender'] ?? '')); ?></span></div>
                                ...
                                <?php if (empty($s_scores)): ?>
                                    <div style="padding:20px;text-align:center;color:#999;"><i class="fas fa-info-circle"></i> No scores recorded for this student yet.</div>
                                <?php else: ?>
                                ...
                                <?php if ((int)($record['show_affective_traits'] ?? 1) && !empty($affective_fields)): ?>
                                    ...
                                    <div class="traits-grid">
                                        <div class="traits-section">
                                            ...
                                            ?>
                                                <div class="trait-row"><span class="t-lbl"><?php echo htmlspecialchars($affective_fields[$fld]); ?></span><span class="trait-val <?php echo $cls; ?>"><?php echo $val ? "{$val} — " . ($trait_labels[$val] ?? $val) : '—'; ?></span></div>
                                            <?php endforeach; ?>
                                        </div>
                                        <div class="traits-section">
                                    ...
                                    <div class="traits-grid">
                                        <div class="traits-section">
                                            ...
                                            foreach (array_slice($pm_keys, 0, $half) as $fld):
                                                ...
                                                $cls = $val ? 'tv-' . strtolower($val) : 'tv-null';
                                            ?>
                                                <div class="trait-row"><span class="t-lbl"><?php echo htmlspecialchars($psychomotor_fields[$fld]); ?></span><span class="trait-val <?php echo $cls; ?>"><?php echo $val ? "{$val} — " . ($trait_labels[$val] ?? $val) : '—'; ?></span></div>
                                            <?php endforeach; ?>
                                        </div>
                                        <div class="traits-section">
                                ...
                                <div class="rc-comments">
                                    <div class="rc-comment-box">
                                        ...
                                        <div class="c-sig"><?php if (!empty($s_comm['class_teachers_name'])): ?>Signed: <strong><?php echo htmlspecialchars($s_comm['class_teachers_name']); ?></strong><?php endif; ?></div>
                                    </div>
                                    <div class="rc-comment-box">
                                        ...
                                        <div class="c-sig"><?php if (!empty($s_comm['principals_name'])): ?>Signed: <strong><?php echo htmlspecialchars($s_comm['principals_name']); ?></strong><?php endif; ?></div>
                                    </div>
                                </div>

                                <!-- Promoted -->
                                <?php if ((int)($record['show_promoted_to'] ?? 1) && $promoted_to): ?>
                                    <div style="padding:8px 16px;background:#e8f5e9;border-top:1px solid #c8e6c9;font-size:.78rem;"><i class="fas fa-arrow-circle-up" style="color:var(--success)"></i> <strong>Promoted to:</strong> <?php echo htmlspecialchars($promoted_to); ?></div>
                                <?php endif; ?>

                                <!-- Footer -->
                                <div class="rc-footer"><span><?php echo htmlspecialchars($school_name); ?> — <?php echo htmlspecialchars($term); ?> Term, <?php echo htmlspecialchars($session); ?></span><span>Generated: <?php echo date('d M Y'); ?></span><span><strong>Status:</strong> <?php echo ucfirst($record['status'] ?? 'draft'); ?></span></div>
                            </div>
                        </div>
                    <?php else: ?>
                        <div style="padding:60px 20px;text-align:center;color:#999;"><i class="fas fa-id-card" style="font-size:3rem;opacity:.3;margin-bottom:14px;display:block"></i>
        ...
        <div style="text-align:center;padding:20px;color:#999;font-size:.8rem;border-top:1px solid var(--light);margin-top:20px" class="no-print">
    ...
    <script>
        ...
        async function downloadReportCardPDF() {
            ...
            try {
                ...
                const finalW = pdfW - 40;
                const finalH = finalW * ratio;
                const y = (pdfH - finalH) / 2;
                pdf.addImage(imgData, 'JPEG', 20, y, finalW, finalH);
                const nameEl = card.querySelector('.rc-bio-val');
                const studentName = nameEl ? nameEl.textContent.trim().replace(/[^a-z0-9_\- ]/gi, '_').replace(/\s+/g, '_') : 'report_card';
